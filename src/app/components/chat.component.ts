  ...
  template: `
    <div class="max-w-4xl mx-auto px-4">
      ...
      >
        ...
        <div
          #chatContainer
          class="h-[550px] overflow-y-auto p-6 space-y-4"
          (scroll)="onScroll()"
        >
          <div
          ...
          <div
            *ngIf="chatService.loading$ | async"
            class="flex justify-center py-4"
          >
            <div class="animate-pulse flex space-x-3">
              <div class="w-2.5 h-2.5 rounded-full bg-primary/70"></div>
              <div
                class="w-2.5 h-2.5 rounded-full bg-primary/70 animation-delay-200"
              ></div>
              <div
                class="w-2.5 h-2.5 rounded-full bg-primary/70 animation-delay-400"
              ></div>
            </div>
          </div>
        </div>
        ...
        <div class="border-t p-4 bg-muted/50">
          <div class="flex space-x-2">
            ...
            <button
              (click)="sendMessage()"
              [disabled]="((loading$ | async) ?? false) || !currentMessage.trim()"
